...
func Test_Set_Get(t *testing.T) {
	...
	assert.Equal(t, []float64{2.1}, (*tree.root.children)[1].items)
	assert.Equal(t, []float64{3.0, 3.1}, (*tree.root.children)[2].items)
	assert.Equal(t, []float64{5.0, 6.0, 7.0}, (*tree.root.children)[3].items)
	tree.Set(8.0)
	...
	deleteData, deleteOk := tree.Delete(8.0)
	assert.True(t, deleteOk)
	assert.Equal(t, []float64{7.0}, (*(*tree.root.children)[1].children)[2].items)
	t.Log(deleteData)

	deleteData, deleteOk = tree.Delete(4.0)
	assert.True(t, deleteOk)
	assert.Equal(t, []float64{3.1, 6.0}, (*tree.root.children)[1].items)
	assert.Equal(t, []float64{3.0}, (*(*tree.root.children)[1].children)[0].items)
	assert.Equal(t, []float64{5.0}, (*(*tree.root.children)[1].children)[1].items)
	t.Log(deleteData)

	deleteData, deleteOk = tree.Delete(3.0)
	assert.True(t, deleteOk)
	assert.Equal(t, []float64{6.0}, (*tree.root.children)[1].items)
	assert.Equal(t, []float64{3.1, 5.0}, (*(*tree.root.children)[1].children)[0].items)
	assert.Equal(t, []float64{7.0}, (*(*tree.root.children)[1].children)[1].items)
	t.Log(deleteData)

	deleteData, deleteOk = tree.Delete(7.0)
	assert.True(t, deleteOk)
	t.Log(tree.root.items)
	t.Log((*tree.root.children)[0].items)
	t.Log((*tree.root.children)[1].items)
	t.Log((*tree.root.children)[2].items)
	// assert.Equal(t, []float64{3.1, 5.0}, (*(*tree.root.children)[1].children)[0].items)
	// assert.Equal(t, []float64{7.0}, (*(*tree.root.children)[1].children)[1].items)
	t.Log(deleteData)
}
